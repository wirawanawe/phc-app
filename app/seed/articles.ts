...
// Export a function to run the seed when imported
export default async function runArticleSeeder() {
  try {
    const seededArticles = await seedArticles();
    console.log(`Seeded ${seededArticles.length} articles successfully`);
    return true;
  } catch (error) {
